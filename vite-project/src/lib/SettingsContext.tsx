...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const setWallpaper = async (file: File | null) => {
    try {
      ...
      if (!file) {
        // Reset to default (Requires backend to handle null, but we'll simulate by not updating or setting a default string)
        // For now, if no file, we just clear local state.
  ...
  return (
    <SettingsContext.Provider value={{ themeColor, setThemeColor, wallpaper, setWallpaper, isLoading }}>
      {children}
